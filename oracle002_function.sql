...
--주어진 문자열에서 왼쪽의 공백제거
SELECT ltrim('   korea'), length('   korea'), ltrim('   korea'), length(ltrim('   korea'))
FROM dual;

--주어진 문자열에서 오른쪽의 공백제거
SELECT rtrim('korea    '), length('korea    '), rtrim('korea    '), length(rtrim('korea    '))
FROM dual;

--주어진 문자열에서 양쪽의 공백제거
SELECT trim('   korea   '), length('   korea   '), trim('   korea   '), length(rtrim('   korea   '))
FROM dual;
...
-- 42523.55를 십의 자리까지 구하시오(반올림)
SELECT round(42523.55,-1)
FROM dual;

-- 42523.55를 일의 자리까지 구하시오(반올림)
...
-- 각 나라의 통화를 표현 줄 때 L 기호를 사용한다.
SELECT to_char(2532, 'L999,999.99')
FROM dual;

--날짜 -> 문자
SELECT to_char(sysdate, 'yyyy-mm-dd hh:mi:ss day')  /*12시간, dy: 월요일*/
FROM dual;
...
/*---------
일반함수(NULL)
2. nul2(컬럼, 대체값1, 대체값2) : 컬럼의 값이 null아니면 대체값1, null이면 대체값2로 출력한다.
------------*/
...
SELECT job_id, salary, decode(job_id, 'PR_REP', salary * 1.05, 
                                      'SA_MAN', salary * 1.1,
                                      'AC_MGR', salary * 1.15, 
                                      'PU_CLERK', salary * 1.2, salary) AS newal
FROM employees;
...
SELECT first_name, hire_date, 
    case when to_char(hire_date, 'mm') <=3 then '1사분기'
         ...
         when to_char(hire_date, 'mm') <=3 then '4사분기'
    end AS "분기"
FROM employees;

-- salary의 값이 10000 미만이면 '하', 10000이상 20000 미만이면 '중', 20000이상이면 '상'
...
/*------------------
집계합수(Aggregate Function), 그룹함수(Group Function)
...
-- 집계함수와 단순컬럼은 함께 사용할 수 없다.(출력 레코드(row)수가 다르기 때문이다.)
...
--50 이하인 부서번호에 대해서 null이 아닌 부서별의 직원수를 출력하시오.
SELECT department_id, count(*)
FROM employees
